...
def get_dataset(prb_def, app, timestamp, train=True, distributed_training=True, tfr_files_bool=False):
    ...
    if train == True:
        ...
        # TODO: OLD STATIC TEST PATH DEFINITION
        tfr_path = ds_path + \
    ...
    if train == True:
        tfr_files_train_ds = tf.data.Dataset.list_files(tfr_files_train, seed=636)
    ...
    print("Scale Output: ", scale_output)
    ...
    print("Loading Model with timestamp: ", timestamp)
    NAME += f'_{train_yp}'+ \
    ...
    if train == True:
        ...
        if tfr_files_bool:
            return dataset_test, X_test, n_samples_tot, model_config, tfr_files
        else:
