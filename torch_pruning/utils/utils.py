...
from ..pruner.algorithms import exp_utils
...
def overall_expressiveness(model, activation_batch):
    batch_activation_map = exp_utils.get_activations_map(model, [nn.Conv2d], activation_batch=activation_batch) 
    ovr_exp = exp_utils.overall_hamming_distance(batch_activation_map)
    return ovr_exp

def expressiveness_map(model, activation_batch):
    batch_activation_map = exp_utils.get_activations_map(model, [nn.Conv2d], activation_batch=activation_batch) 
    #ovr_exp = exp_utils.overall_hamming_distance(batch_activation_map)
    return expressiveness_map