...
# TODO: WRITE EXPRESSIVE PROCEDURES CALCULATIONS
class ExpressiveImportance(Importance):
    # target_types =
    #   1. mobilenetv2: [nn.modules.conv._ConvNd, nn.Linear, nn.modules.batchnorm._BatchNorm]
    #   2. resnet56 : [nn.modules.conv._ConvNd, nn.Linear]
    ...
    def _reduce(self, group_imp: typing.List[torch.Tensor], group_idxs: typing.List[typing.List[int]]):
        ...
        for i, (imp, root_idxs) in enumerate(zip(group_imp, group_idxs)):
            if self.group_reduction == "sum" or self.group_reduction == "mean":
                reduced_imp = reduced_imp.to(imp.dtype) # TODO: REMOVE
                reduced_imp.scatter_add_(0, torch.tensor(root_idxs, device=imp.device), imp) # accumulated importance
            elif self.group_reduction == "max": # keep the max importance
                selected_imp = torch.index_select(reduced_imp, 0, torch.tensor(root_idxs, device=imp.device))
                selected_imp = torch.maximum(input=selected_imp, other=imp)
    ...
    @torch.no_grad()
    def __call__(self, group: Group, expressivity_scores: dict={}, ch_groups: int=1, return_group_size=False): #  
        group_imp = []
        group_idxs = []
        group_size = 0
        #print(group)
        #print(expressivity_scores.keys())
        # Iterate over all groups and estimate group importance
        for i, (dep, idxs) in enumerate(group):
            layer = dep.layer
            #print("LAYER:", layer)
            ...
            root_idxs = group[i].root_idxs
            #print("root_idxs:", root_idxs)
            
            # Source and Target are propably different components (example, prune_out_channels on layer3.0.downsample.0 (Conv2d(32, 64, kernel_size=(1, 1), stride=(2, 2), bias=False)) => prune_out_channels on layer3.0.downsample.1 (BatchNorm2d(64, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)), #idxs=64)
            module_key_source = dep.source.name.split(" ")[0]
            module_key_target = dep.target.name.split(" ")[0]
            ...
            if prune_fn in [
                ...
                #print("module_key_source", module_key_source)
                #print(expressivity_scores.get(module_key_source))
                #print(dep) # TODO: REMOVE
                #if module_key_source not in expressivity_scores:
                if expressivity_scores.get(module_key_source) == None:
                    #print("Skipped Layer:", layer)
                    continue
                
                #print(expressivity_scores.keys())
                local_imp = expressivity_scores[module_key_source]
                #print("kernel_imp: ", local_imp.dtype)
                #print("root_idxs: ", group_idxs)
                group_imp.append(local_imp) # TODO: INVESTIGATE
                group_idxs.append(root_idxs)
                #print(module_key_source)
                ...
                print("dep:", dep)
                print("layer: ", layer)
                print("Local Importance Raw format:", local_imp.dtype)
                exit()
            ...
            # Expressivity metric cannot be used to estimated the in_channels of a layer directly but instead can only be used 
            # to calculate the out_channel. In layers are pruned as a propagated result of pruning out_channels in this implementation.
        ...
        if len(group_imp) == 0: # skip groups without parameterized layers
            if return_group_size:
                return None, 0
            return None
        #print("local_imp: ", group_imp)
        #print("root_idxs: ", group_idxs)
        
        #print("1.", len(group_imp[0]))
        #print("1.Group Size", group_size)
        
        group_imp = self._reduce(group_imp, group_idxs) # TODO: INVESTIGATE
        #print("2.", group_imp.shape) # group_imp type == torch.tesnor
        #print("2.Group Size", group_size)
        #print("Group Importance Before Normalization:", group_imp)
        group_imp = self._normalize(group_imp, self.normalizer)
        #print("Group Importance After Normalization:", group_imp)
        #print("3.", len(group_imp))
        #print("3.Group Size", group_size)
        if return_group_size:
            #print("GROUP SIZE IS ON!")
