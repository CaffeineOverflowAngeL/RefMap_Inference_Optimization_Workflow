...
# Our imports
from .exp_utils import get_activations_map, hamming_distance_batch, kernel_expressivity_scores, hamming_distance_matrix_2, hamming_distance_batch_2
...
class HybridMetaPruner:
    ...
    def __init__(
        ...
        activation_batch: torch.Tensor, # a batch of n samples used to define the activations map of the given model.
        importance: typing.Callable, # tp.importance.Importance for group importance estimation
        expressiveness: typing.Callable, #tp.importance.Importance for group expressivity estimation
        ...
        # Hybrid Pruning
        exp_component: float = 0.5, # the impact factor of expressivity on the pruning criteria.
        imp_component: float = 0.5, # the impact factor of importance on the pruning criteria.
        
        # deprecated
        channel_groups: typing.Dict[nn.Module, int] = dict(), # channel groups for layers
    ):
        self.model = model
        self.activation_batch = activation_batch
        ...
        # Hybrid Pruning
        self.exp_component = exp_component
        self.imp_component = imp_component
    ...
    def estimate_importance(self, group, expressivity_scores, ch_groups=1) -> torch.Tensor:
        return (self.imp_component * self.importance(group, ch_groups=ch_groups)) + (self.exp_component * self.expressiveness(group, expressivity_scores, ch_groups=ch_groups))
    
    def estimate_importance_regularize(self, group, ch_groups=1) -> torch.Tensor:
    ...
    def prune_local(self) -> typing.Generator:
        ...
        device = self.activation_batch.get_device()
        #print("Device: ", device) # TODO: REMOVE
        batch_activation_map = get_activations_map(self.model, [nn.Conv2d], activation_batch=self.activation_batch) 
        #"Activation Map ON: ", batch_activation_map.keys()) # TODO: REMOVE
        batch_activation_scores_array = hamming_distance_batch(batch_activation_map, device=device, normalize=True) # TODO: EXTEND COMPATIBILITY FOR ALL DEVICES (REGARDING THE NORMALIZATION)
        #print("HD Scores ON: ", batch_activation_scores_array.keys())
        expressivity_scores = kernel_expressivity_scores(batch_activation_scores_array, device=device) # TODO: EXTEND COMPATIBILITY FOR ALL LAYERS AND ALL DEVICES
        ...
        for group in self.DG.get_all_groups(ignored_layers=self.ignored_layers, root_module_types=self.root_module_types):
            if self._check_sparsity(group): # check pruning ratio
                ...
                imp = self.estimate_importance(group, ch_groups=ch_groups, expressivity_scores=expressivity_scores)
    ...
    def prune_global(self) -> typing.Generator:
        ...
        device = self.activation_batch.get_device()
        batch_activation_map = get_activations_map(self.model, [nn.Conv2d], activation_batch=self.activation_batch) 
        batch_activation_scores_array = hamming_distance_batch_2(batch_activation_map, device=device, normalize=True)
        expressivity_scores = kernel_expressivity_scores(batch_activation_scores_array, device=device) # TODO: EXTEND COMPATIBILITY FOR ALL LAYERS AND ALL DEVICES
        ...
        for group, ch_groups, imp in global_importance:
            ...
            if ch_groups > 1: # re-compute importance for each channel group if channel grouping is enabled
                ...
                imp = self.estimate_importance(group, ch_groups=ch_groups, expressivity_scores=expressivity_scores) # re-compute importance
